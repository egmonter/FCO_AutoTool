...
def _centos_reached_linux_banner(s: SVOSSession) -> bool:
    """Returns True if the serial buffer looks like it reached the CentOS/Linux banner."""
    try:
        text = _strip_ansi(s.buf).upper()
    except Exception:
        text = str(s.buf).upper()
